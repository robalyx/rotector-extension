<script lang="ts">
	...
	async function loadWarMap() {
		...
		try {
			...
			error = err instanceof Error ? err.message : $_('warzone_overview_error_load');
		} finally {
			isLoading = false;
		}
	}
	...
	async function handleFindReportableUser() {
		...
		try {
			...
			if (error.status === 404) {
				findUserError = $_('warzone_overview_error_no_users');
			} else if (error.status === 403) {
				findUserError = $_('warzone_overview_error_auth_expired');
			} else {
				findUserError = error.message || $_('warzone_overview_error_find_user');
			}
		} finally {
			isFindingUser = false;
		}
...
{#if isLoading}
	<div class="war-zone-loading">
		<LoadingSpinner />
		<p class="war-zone-loading-text">{$_('warzone_overview_loading')}</p>
	</div>
{:else if error}
...
{:else if warMap}
	<div class="war-zone-overview">
		...
		<div class="zones-section">
			<h4 class="section-title">{$_('warzone_overview_section_zones')}</h4>
		...
		<div class="find-user-section">
			...
			>
				{#if isFindingUser}
					...
					{$_('warzone_overview_finding_user_loading')}
				{:else}
					{$_('warzone_overview_button_find_user')}
				{/if}
			</button>
			{#if findUserError}
		...
		{#if warMap.activeTargets && warMap.activeTargets.length > 0}
			<div class="targets-section">
				<h4 class="section-title">{$_('warzone_overview_section_targets')}</h4>
		...
		<div class="war-zone-stats-card">
			<h4 class="war-zone-stats-title">{$_('warzone_overview_section_global_stats')}</h4>
			<div class="war-zone-stats-grid overview-stats-grid">
				<div class="war-zone-stat-item">
					...
					<span class="war-zone-stat-label">{$_('warzone_overview_stat_total_zones')}</span>
				</div>
				<div class="war-zone-stat-item">
					<span class="war-zone-stat-value">{warMap.globalStats.averageLiberation.toFixed(1)}%</span
					>
					<span class="war-zone-stat-label">{$_('warzone_overview_stat_avg_liberation')}</span>
				</div>
				<div class="war-zone-stat-item">
					<span class="war-zone-stat-value">{warMap.globalStats.totalBanned.toLocaleString()}</span>
					<span class="war-zone-stat-label">{$_('warzone_overview_stat_total_banned')}</span>
				</div>
				<div class="war-zone-stat-item">
					<span class="war-zone-stat-value">{warMap.globalStats.totalTargets.toLocaleString()}</span
					>
					<span class="war-zone-stat-label">{$_('warzone_overview_stat_active_targets')}</span>
				</div>
			</div>
		</div>
		...
		<div class="war-zone-stats-card">
			<h4 class="war-zone-stats-title">{$_('warzone_overview_section_global_trends')}</h4>
		...
		{#if warMap.majorOrders && warMap.majorOrders.length > 0}
			<div class="major-orders-section">
				<h4 class="section-title">{$_('warzone_overview_section_major_orders')}</h4>
