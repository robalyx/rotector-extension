...
import { FRIENDS_CAROUSEL_SELECTORS, FRIENDS_SELECTORS } from '../../controllers/selectors/friends';
import { GROUP_HEADER_SELECTORS, GROUPS_MODAL_SELECTORS } from '../../controllers/selectors/groups';
import {
	BTROBLOX_GROUPS_SELECTORS,
	PROFILE_GROUPS_SHOWCASE_SELECTORS,
	PROFILE_SELECTORS
} from '../../controllers/selectors/profile';
import { SEARCH_SELECTORS } from '../../controllers/selectors/search';
import { logger } from '../logging/logger';

// URL pattern -> PageType lookup table consumed by detectPageType
const PAGE_PATTERNS: ReadonlyArray<{ pattern: RegExp; type: PageType; hash?: string }> = [
...
export function detectPageType(url: string): PageType | null {
	try {
		...
		for (const { pattern, type, hash } of PAGE_PATTERNS) {
...
// Walks up from the anchor to classify the surrounding card or page, falling back to URL match
...
// Reads displayName, username and avatar from the container using per-pageType selectors
...
// Picks BTRoblox vs native group selectors so both DOM variants resolve the same fields
