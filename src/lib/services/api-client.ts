import { API_CONFIG, API_ACTIONS, MESSAGES, VOTE_TYPES, type VoteType } from '../types/constants';
import type { 
  UserStatus,
  VoteData,
  VoteResult,
...
class RotectorApiClient {
    ...
    while (attempt <= maxRetries) {
      try {
        ...
        if (response && response.success) {
          ...
          const responseWithError = response as ApiResponse & { requestId?: string; code?: string; type?: string };
  ...
  private isRetryableError(error: Error): boolean {
    ...
    // Rate limits (if we can detect them)
