  ...
  browser.runtime.onMessage.addListener(
    (
      ...
      void (async () => {
        try {
          const handler =
            actionHandlers[request.action as keyof typeof actionHandlers];
          ...
          logger.error(
            `Background script error for action ${request.action}:`,
            error,
          );
      ...
      void (async () => {
        try {
          if (
            ...
            if (!message.state) {
              logger.error(
                "Discord OAuth callback missing required state parameter",
              );
              if (sender.tab?.id) {
            ...
            if (result.oauthInProgress) {
              logger.warn(
                "OAuth callback already in progress, rejecting duplicate request",
              );
              if (sender.tab?.id) {
                await browser.tabs.remove(sender.tab.id);
              }
            ...
            try {
              ...
              if (
                ...
                logger.error("OAuth state validation failed", {
                  ...
                  stateMatch:
                    (result.discordOAuthState as string) === message.state,
                  isExpired,
                });
              ...
              try {
                ...
                logger.warn(
                  "Failed to notify components about Discord authentication:",
                  error,
                );
              }
          ...
          logger.error(
            "Discord OAuth external message handling failed:",
            error,
          );
        }
      })();
