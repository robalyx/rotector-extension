    ...
    function handleDismiss() {
        ...
        setTimeout(() => {
            dismissAdvancedViolationBanner().catch((error) => {
                logger.error(
                    "Failed to dismiss advanced violation banner:",
                    error,
                );
                isClosing = false;
            });
    ...
    async function handleGoToSetting() {
        try {
            ...
            setTimeout(() => {
                ...
                settingsSection?.highlightSetting(
                    SETTINGS_KEYS.ADVANCED_VIOLATION_INFO_ENABLED,
                );
            }, 150);
        } catch (error) {
            logger.error(
                "Failed to navigate to advanced violation setting:",
                error,
            );
        }
    }
