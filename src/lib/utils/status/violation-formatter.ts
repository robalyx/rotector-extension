...
// Outfit reasons get pipe-parsed into structured entries, others stay as raw strings
export function formatViolationReasons(
	...
	return Object.entries(reasons).map(([reasonType, reason]) => ({
		...
		...(reason.evidence?.length && {
			evidence: formatEvidence(reason.evidence, reasonType === REASON_KEYS.AVATAR_OUTFIT)
		})
	}));
}
