...
import {API_CONFIG, INTEGRATION_SOURCES, STATUS, VOTE_TYPES, type VoteType} from '@/lib/types/constants';
...
export async function checkMultipleGroups(groupIds: Array<string | number>): Promise<GroupStatus[]> {
...
export async function queueUser(userId: string | number, inappropriateOutfit: boolean = false, inappropriateProfile: boolean = false, inappropriateFriends: boolean = false, inappropriateGroups: boolean = false): Promise<QueueResult> {
    const sanitizedUserId = validateEntityId(userId);
