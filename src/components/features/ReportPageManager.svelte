<script lang="ts">
    ...
    // Component state
    let reportHelper: { element: HTMLElement; cleanup: () => void } | null =
        null;
    ...
    async function handleReportSubmit(): Promise<void> {
        ...
        try {
            ...
            await apiClient.submitExtensionReport(
                parseInt(userId),
                reportReason,
            );
        ...
        try {
            ...
            const inappropriateLanguageOption = Array.from(
                document.querySelectorAll(
                    REPORT_PAGE_SELECTORS.DROPDOWN_OPTION,
                ),
            ).find((option) => {
                const textElement = option.querySelector("span");
                return textElement?.textContent?.includes(
            ...
            if (inappropriateLanguageOption) {
                ...
                logger.warn(
                    "Could not find Inappropriate Language category option",
                );
            }
        } catch (error) {
    ...
    async function mountReportHelper(): Promise<void> {
        try {
            ...
            if (!userStatus || !hasUserProfileViolations()) {
                logger.debug(
                    "User has no profile violations, not showing report helper",
                );
                return;
            }
            ...
            const formContainer = document.querySelector(
                REPORT_PAGE_SELECTORS.FORM_CONTAINER,
            );
    ...
    async function handleFillForm(): Promise<void> {
        try {
            ...
            if (
                ...
                profileReason.evidence.forEach(
    ...
    function showSuccessMessage(): void {
        try {
            ...
            successMessageElement.className =
                COMPONENT_CLASSES.REPORT_HELPER_SUCCESS_MESSAGE;
