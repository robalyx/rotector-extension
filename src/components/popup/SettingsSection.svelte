...
{#if showUnlockMessage}
  <div
      class="mb-2 p-2 bg-green-100 dark:bg-green-900 border border-green-300 dark:border-green-700 rounded text-sm text-green-800 dark:text-green-200">
...
>
  {#each SETTING_CATEGORIES as category (category.title)}
    <fieldset style:border-color="var(--color-border-subtle)"
              class="border rounded-sm p-2 px-3 pb-3 m-0 mb-3 last:mb-0">
      <legend
          class="text-xs font-medium text-text-subtle dark:text-text-subtle-dark px-1 ml-0.5">{category.title}</legend>
                  ...
                  <div class="flex items-center gap-1">
                    ...
                    {#if setting.key === SETTINGS_KEYS.ROTECTOR_INTEGRATION_ENABLED}
                      <a
                          ...
                          style:color="white"
                          class="inline-flex items-center justify-center w-4 h-4 rounded-sm transition-all duration-200 hover:scale-110"
  ...
  {#if $settings[SETTINGS_KEYS.DEVELOPER_MODE_UNLOCKED]}
    <fieldset style:border-color="var(--color-border-subtle)"
              class="border rounded-sm p-2 px-3 pb-3 m-0 mb-3 last:mb-0 bg-yellow-50 dark:bg-yellow-900/20">
...
>
  ...
  <div class="modal-content-section-recommendation">
    <p class="text-sm leading-relaxed m-0"><strong>Recommendation:</strong> Only enable this if you need detailed
