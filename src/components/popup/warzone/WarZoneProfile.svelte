<script lang="ts">
	...
	function getDiscordAvatarUrl(avatarHash: string | null, userId: string): string | null {
			...
			<div class="war-zone-detail-item">
				...
				<div class="discord-info">
					{#if $authStore.profile.discordAvatar && $authStore.profile.discordUserId}
						...
						/>
					{/if}
					<span class="war-zone-detail-value">{$authStore.profile.discordUsername}</span>
				</div>
			</div>
						...
						{:else}
							<label class="war-zone-toggle-switch">
								...
								<span class="war-zone-toggle-label"
									>{isAnonymousInput ? 'Anonymous' : 'Public'}</span
								>
							</label>
						{/if}
					</div>
