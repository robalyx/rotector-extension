<script lang="ts">
    ...
    const chartData: ChartDataPoint[] = $derived.by(() => {
        if (selectedChart === 'cost') {
            ...
            return weeksData.map((week, index) => ({
                ...
                height: Math.max((week.totalCost / maxValue) * (CHART_HEIGHT - MARGIN.top - MARGIN.bottom), 2),
                x: MARGIN.left + 15 + index * (BAR_WIDTH + BAR_SPACING),
                y: CHART_HEIGHT - MARGIN.bottom,
                color: 'var(--color-primary)',
                details: [
                    {label: 'Total Requests', value: formatNumber(week.totalRequests)},
                    {label: 'Period', value: formatDateRange(week.startDate, week.endDate)}
                ]
            }));
            ...
            return weeksData.map((week, index) => ({
                ...
                height: Math.max((week.totalRequests / maxValue) * (CHART_HEIGHT - MARGIN.top - MARGIN.bottom), 2),
                x: MARGIN.left + 15 + index * (BAR_WIDTH + BAR_SPACING),
                y: CHART_HEIGHT - MARGIN.bottom,
                color: 'var(--color-success)',
                details: [
                    {label: 'Total Cost', value: formatCurrency(week.totalCost)},
                    {label: 'Period', value: formatDateRange(week.startDate, week.endDate)}
                ]
            }));
        } else {
            // tokens - display as stacked bars
            const maxValue = Math.max(1, ...weeksData.map(w => w.totalPromptTokens + w.totalCompletionTokens + w.totalReasoningTokens));
            return weeksData.map((week, index) => {
                const total = week.totalPromptTokens + week.totalCompletionTokens + week.totalReasoningTokens;
                const baseHeight = (total / maxValue) * (CHART_HEIGHT - MARGIN.top - MARGIN.bottom);
                const promptHeight = total === 0 ? 0 : Math.max((week.totalPromptTokens / total) * baseHeight, 1);
                const completionHeight = total === 0 ? 0 : Math.max((week.totalCompletionTokens / total) * baseHeight, 1);
                const reasoningHeight = total === 0 ? 0 : Math.max((week.totalReasoningTokens / total) * baseHeight, 1);
                ...
                return {
                    ...
                    color: 'var(--color-primary)',
                    stacked: {
                        prompt: {height: promptHeight, color: 'var(--color-primary)'},
                        completion: {height: completionHeight, color: 'var(--color-success)'},
                        reasoning: {height: reasoningHeight, color: 'var(--color-warning)'}
                    },
                    details: [
                        {label: 'Prompt Tokens', value: formatNumber(week.totalPromptTokens)},
                        {label: 'Completion Tokens', value: formatNumber(week.totalCompletionTokens)},
                        {label: 'Reasoning Tokens', value: formatNumber(week.totalReasoningTokens)},
                        {label: 'Total Cost', value: formatCurrency(week.totalCost)},
                        {label: 'Period', value: formatDateRange(week.startDate, week.endDate)}
                    ]
                };
    ...
    function handleBarHover(event: MouseEvent, data: ChartDataPoint) {
        ...
        // Position tooltip within viewport boundaries
        const tooltipX = Math.max(80, Math.min(svgRect.left + data.x + (BAR_WIDTH / 2), window.innerWidth - 80));
        const rawY = svgRect.top + (data.y - data.height - 10);
        const tooltipY = Math.max(12, Math.min(rawY, window.innerHeight - 12));
    ...
    function getChartLabel(): string {
        switch (selectedChart) {
            case 'cost': return 'Weekly AI Costs';
            case 'requests': return 'Weekly AI Requests';
            case 'tokens': return 'Total AI Tokens';
            default: return '';
        }
    }
        ...
        >
            ...
            {#each [0, 0.25, 0.5, 0.75, 1] as gridLevel (gridLevel)}
                <line
                    ...
                    y1={CHART_HEIGHT - MARGIN.bottom - (gridLevel * (CHART_HEIGHT - MARGIN.top - MARGIN.bottom))}
                    y2={CHART_HEIGHT - MARGIN.bottom - (gridLevel * (CHART_HEIGHT - MARGIN.top - MARGIN.bottom))}
                />
            {/each}
                ...
                <g class="bar-group">
                    {#if selectedChart === 'tokens' && data.stacked}
                        ...
                        <rect
                            ...
                            aria-label="Prompt tokens for {data.week}: {formatNumber(weeksData[index].totalPromptTokens)}"
                        ...
                        <rect
                            class="chart-bar stacked-bar"
                            aria-label="Completion tokens for {data.week}: {formatNumber(weeksData[index].totalCompletionTokens)}"
                        ...
                        <rect
                            class="chart-bar stacked-bar"
                            aria-label="Reasoning tokens for {data.week}: {formatNumber(weeksData[index].totalReasoningTokens)}"
