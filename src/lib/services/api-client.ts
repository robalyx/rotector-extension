...
class RotectorApiClient {
	...
	async checkMultipleUsers(
		...
		for (let i = 0; i < batches.length; i++) {
			...
			try {
				...
				logger.error(`Failed to process batch ${i + 1}/${batches.length}:`, error);
				throw error;
			}
		}
	...
	async checkMultipleGroups(
		...
		for (let i = 0; i < batches.length; i++) {
			...
			try {
				...
				logger.error(`Failed to process group batch ${i + 1}/${batches.length}:`, error);
				throw error;
			}
		}
	...
	async getQueueLimits(options?: RequestOptions): Promise<QueueLimitsData> {
		return this.sendMessage<QueueLimitsData>(API_ACTIONS.GET_QUEUE_LIMITS, {}, options);
	}
	...
	async getVotes(userId: string | number, options?: RequestOptions): Promise<VoteData> {
		...
		return this.sendMessage<VoteData>(API_ACTIONS.GET_VOTES, { userId: sanitizedUserId }, options);
	}
	...
	async getMultipleVotes(
		...
		for (let i = 0; i < batches.length; i++) {
			...
			try {
				...
				logger.error(`Failed to process vote batch ${i + 1}/${batches.length}:`, error);
				throw error;
			}
		}
	...
	async getStatistics(options?: RequestOptions): Promise<Statistics> {
		return this.sendMessage<Statistics>(API_ACTIONS.GET_STATISTICS, {}, options);
	}
	...
	async getExtensionProfile(options?: RequestOptions): Promise<ExtensionUserProfile> {
		return this.sendMessage<ExtensionUserProfile>(API_ACTIONS.EXTENSION_GET_PROFILE, {}, options);
	}
	...
	async resetUuid(options?: RequestOptions): Promise<{ uuid: string; message: string }> {
	...
	async getExtensionStatistics(options?: RequestOptions): Promise<ExtensionStatistics> {
		return this.sendMessage<ExtensionStatistics>(API_ACTIONS.EXTENSION_GET_STATISTICS, {}, options);
	}

	// Gets a random reportable user for the authenticated extension user
	async getReportableUser(options?: RequestOptions): Promise<ReportableUserResponse> {
	...
	async getWarOrders(options?: RequestOptions): Promise<MajorOrder[]> {
		return this.sendMessage<MajorOrder[]>(API_ACTIONS.WAR_GET_ORDERS, {}, options);
	}

	// Gets detailed information about a specific major order
	async getWarOrder(orderId: number, options?: RequestOptions): Promise<MajorOrder> {
		if (!orderId || orderId <= 0) {
			throw new Error('Invalid order ID provided.');
		}
		return this.sendMessage<MajorOrder>(API_ACTIONS.WAR_GET_ORDER, { orderId }, options);
	}

	// Gets 30 days of global historical statistics
	async getGlobalStatisticsHistory(options?: RequestOptions): Promise<GlobalHistoricalStats> {
		return this.sendMessage<GlobalHistoricalStats>(API_ACTIONS.WAR_GET_STATS_HISTORY, {}, options);
	}
	...
	async getWarZone(zoneId: number, options?: RequestOptions): Promise<ZoneDetails> {
		if (typeof zoneId !== 'number' || zoneId < 0) {
			throw new Error('Invalid zone ID provided.');
		}
		return this.sendMessage<ZoneDetails>(API_ACTIONS.WAR_GET_ZONE, { zoneId }, options);
	}
