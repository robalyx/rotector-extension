<script lang="ts">
    ...
    import type { GlobalHistoricalStats } from "@/lib/types/api";
    ...
    let historicalData = $state<GlobalHistoricalStats | null>(null);
        ...
        try {
            ...
            error =
                err instanceof Error
                    ? err.message
                    : "Failed to load global historical data";
        } finally {
            isLoading = false;
        }
    }
    ...
    const chartData: ChartDataPoint[] = $derived.by(() => {
        if (
            ...
            historicalData.dailyStats.length === 0
        )
            return [];
        ...
        if (selectedChart === "liberation") {
            ...
            return stats.map((stat, index) => ({
                ...
                details: [
                    ...
                    {
                        label: "Banned",
                        value: stat.bannedUsers.toLocaleString(),
                    },
                ],
            }));
            ...
            return stats.map((stat, index) => {
                ...
                const confirmedHeight =
                    total === 0
                        ? 0
                        : Math.max(
                              (stat.confirmedUsers / total) * baseHeight,
                              1,
                          );
            ...
            return stats.map((stat, index) => {
                const banRate =
                    stat.totalUsers > 0
                        ? (stat.bannedUsers / stat.totalUsers) * 100
                        : 0;
                return {
                    date: formatDate(stat.date),
                    value: banRate,
    ...
    function getChartLabel(): string {
        switch (selectedChart) {
            case "liberation":
                return "Global Liberation";
            case "users":
                return "Total Users";
            case "banRate":
                return "Global Ban Rate";
            default:
                return "";
        }
    }
                    ...
                    {#if chartData.length > 0}
                        <text
                            ...
                            y={chartData[chartData.length - 1].y -
                                chartData[chartData.length - 1].height -
                                8}
                        >
                        ...
                        <g class="stacked-group">
                            {#if data.stacked}
                                ...
                                <rect
                                    ...
                                    x={data.x - 4}
                                    y={data.y -
                                        data.stacked.banned.height -
                                        data.stacked.flagged.height}
                                />
                                <rect
                                    class="war-zone-chart-stacked"
